...
LiveTileParser::LiveTileParser(mapbox::util::geojsonvt::Tile& in_tile_, LiveTileData &tile_,
...
void LiveTileParser::parse() {
    parseStyleLayers(style->layers);
}

bool LiveTileParser::obsolete() const { return tile.state == TileData::State::obsolete; }

void LiveTileParser::parseStyleLayers(util::ptr<const StyleLayerGroup> group) {
    if (!group) {
        return;
    }
...
std::unique_ptr<Bucket> LiveTileParser::createBucket(const StyleBucket &bucket_desc) {
...
//std::unique_ptr<Bucket> LiveTileParser::createSymbolBucket(const mapbox::util::geojsonvt::TileFeature &feature,
//                                                       const StyleBucket &bucket_desc) {
...
//    bucket->addFeatures(
//                        feature, bucket_desc.filter, tile.id, spriteAtlas, *sprite, glyphAtlas, glyphStore);
//    return obsolete() ? nullptr : std::move(bucket);
//}
}
